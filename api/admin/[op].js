...
async function op_create_user(actorId, body) {
  ...
  const u = await sb_admin('POST', '/admin/users', {
    ...
    user_metadata: { full_name, username, role, must_change_password: true },
  });
  const new_id = u.id;
  ...
  if (product_ids.length) {
    await sb_rest('POST', '/user_products', product_ids.map(pid => ({
      user_id: new_id, product_id: pid, assigned_by: actorId,
    })), { prefer: 'resolution=ignore-duplicates' });
  }

  await audit(actorId, 'create_user', new_id, null,
    { email, role, products: product_ids });

  return { ok: true, user_id: new_id, email, temp_password: password };
}
...
async function op_reset_password(actorId, body) {
  ...
  await sb_rest('PATCH', `/profiles?id=eq.${user_id}`, { must_change_password: true });
  await audit(actorId, 'reset_password', user_id);
  return { ok: true, user_id, temp_password: password };
}
...
async function op_set_active(actorId, body, active) {
  ...
  await sb_rest('PATCH', `/profiles?id=eq.${user_id}`, { is_active: active });
...
async function op_assign_product(actorId, body) {
  ...
  await sb_rest('POST', '/user_products',
    [{ user_id, product_id, assigned_by: actorId }],
    { prefer: 'resolution=ignore-duplicates' });
...
async function op_unassign_product(actorId, body) {
  const { user_id, product_id } = body;
  if (!user_id || !product_id) throw new Error('user_id & product_id required');
  await sb_rest('DELETE',
    `/user_products?user_id=eq.${user_id}&product_id=eq.${encodeURIComponent(product_id)}`);
...
async function op_update_products(actorId, body) {
  ...
  if (product_ids.length) {
    await sb_rest('POST', '/user_products', product_ids.map(pid => ({
      user_id, product_id: pid, assigned_by: actorId,
    })), { prefer: 'resolution=ignore-duplicates' });
  }
  await audit(actorId, 'update_products', user_id, null, { products: product_ids });
  return { ok: true, product_ids };
}
...
async function op_set_role(actorId, body) {
  ...
  await sb_rest('PATCH', `/profiles?id=eq.${user_id}`, { role });
  await audit(actorId, 'set_role', user_id, null, { role });
  return { ok: true, role };
}
...
export default async function handler(req, res) {
  ...
  if (op === 'health') {
    return res.status(200).json({ ok: true, service: 'immuvi-admin', mode: 'vercel' });
  }
  ...
  try {
    let result;
    switch (op) {
      case 'create-user':       result = await op_create_user(actorId, body); break;
      case 'reset-password':    result = await op_reset_password(actorId, body); break;
      case 'deactivate':        result = await op_set_active(actorId, body, false); break;
      case 'reactivate':        result = await op_set_active(actorId, body, true); break;
      case 'assign-product':    result = await op_assign_product(actorId, body); break;
      case 'unassign-product':  result = await op_unassign_product(actorId, body); break;
      case 'update-products':   result = await op_update_products(actorId, body); break;
      case 'set-role':          result = await op_set_role(actorId, body); break;
      case 'delete-user':       result = await op_delete_user(actorId, body); break;
      default:
        return res.status(404).json({ error: `unhandled op: ${op}` });
    }
