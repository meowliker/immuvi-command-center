...
export function findLinkedAd(action, ads) {
  ...
  if (sourceAdId) {
    ...
    if (bySourceId && !actionSourceLinkLooksCorrupt(action, bySourceId)) return bySourceId;
  }
...
function actionSourceLinkLooksCorrupt(action, ad) {
  const actionClickupId = actionClickUpId(action);
  const adClickupId = text(ad && (ad.clickupTaskId || ad._clickupId));
  if (!action || !ad || !actionClickupId || !adClickupId || actionClickupId === adClickupId) return false;

  const actionTitle = normalizeTaskName(action.title || action.taskName);
  const adTitle = normalizeTaskName(ad.formatName || ad.taskName || ad.title);
  if (!actionTitle || !adTitle) return false;
  return !(actionTitle === adTitle || actionTitle.startsWith(adTitle) || adTitle.startsWith(actionTitle));
}
