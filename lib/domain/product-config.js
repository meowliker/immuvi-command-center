...
export function buildProductConfigForUpsert(product, existingConfig = {}) {
  const config = normalizeProductConfig(existingConfig);
  const incoming = {
    clickup_list_id: product && product.clickupListId,
    clickup_list_name: product && product.clickupListName,
    color: product && product.color,
    ins_prefix: product && product.insPrefix,
    created_at_ms: product && product.createdAt,
    last_synced_at_ms: product && product.lastSyncedAt,
    last_synced_count: product && product.lastSyncedCount,
  };

  for (const [key, value] of Object.entries(incoming)) {
    if (value === '__clear__') {
      config[key] = key === 'last_synced_count' ? 0 : '';
      continue;
    }

    if (value === undefined || value === null) continue;
    if (value === '' && config[key]) continue;
    if (value === 0 && config[key] && key === 'last_synced_count') continue;
    config[key] = value;
  }
