...
class Worker:
                    ...
                    try:
                        ...
                        for _o in _outputs:
                            ...
                            if any(m in _model for m in _bad_markers):
                                ...
                                break
                        if not _is_fake and len(_outputs) > 1:
                            _sorted_outputs = sorted(_outputs, key=_output_variation_number)
                            if _sorted_outputs != _outputs:
                                _finish_producer_run(
                                    supabase_url=os.environ["SUPABASE_URL"],
                                    service_key=os.environ["SUPABASE_SERVICE_ROLE_KEY"],
                                    run_id=run_id, status="done",
                                    outputs=_sorted_outputs, error=None,
                                )
                                log(f"[producer] run {run_id} normalized output order by variation id")
    ...
    def run_skill_on_job(self, job: dict) -> dict:
        ...
        prompt = (
            ...
            f"     Plus: body_copy, caption_timeline, creative_hypothesis, duration_seconds (best-effort), and voice_over.\n"
            f"     caption_timeline is MANDATORY: visible captions/overlays with time ranges, split whenever caption text changes.\n"
            f"     voice_over is MANDATORY: only spoken narration. Never copy standalone visible captions or hook text into voice_over. "
            f"If there is no spoken narration, write exactly 'No voice over'. If speech is present but exact words cannot be verified, "
            f"write exactly 'Voice over present - transcript unavailable'.\n"
            ...
            f"a dedicated 'Voice Over' line with the transcript, 'No voice over', or 'Voice over present - transcript unavailable'. "
            ...
            f"Add section ## 8\\. NEXT AD CAPTION PLAN with exact captions to use next, what to change, and timing/order.\n"
            ...
            f"    data->>'voiceOver'         non-empty (spoken transcript, 'No voice over', or 'Voice over present - transcript unavailable')\n"
    ...
    def run_variation_brief_skill_on_job(self, job, ctx):
        ...
        prompt = (
            ...
            f"     replication_brief, what_to_test, competitor_intel, our_next_ad, next_ad_caption_plan.\n"
            ...
            f"       ## 8\\. NEXT AD CAPTION PLAN  (exact captions to use next + timing/order + what to change)\n"
            f"\n"
