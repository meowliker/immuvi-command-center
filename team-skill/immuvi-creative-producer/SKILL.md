...
3. Extract the creative brief.
   ...
   - **REFERENCE FIDELITY CONTRACT (mandatory for every concept with a reference image).** The output must preserve the reference's format anatomy, not just its broad theme. Before generating, visually inspect the inspiration and write a compact `reference_anatomy` note with:
     ...
     - `product_element_map`: every reference element that carries the source product/category, including product names, logos, packshots, UI/app screenshots, workbook/activity names, icons, props, card titles, demonstrations, CTA wording, and offer badges. For each one, write the required replacement anchored to the canonical product, angle, persona, and offer.
   ...
   - **Hard fidelity rules:**
     ...
     - Product adaptation is mandatory for visuals, not just text. Keep the reference slots and style, but replace every source-product visual with an equivalent for the canonical product. For example, if the reference is an art-therapy worksheet and the target product is `Kids Mental Health`, keep the listicle/card layout but replace art/coloring activities and art-supply props with mental-health calm tools, feeling check-ins, coping skills, breathing prompts, worry routines, parent/teacher support cues, and other product-relevant visuals. Do not keep source-category artifacts such as crayons, paintbrushes, mandalas, yoga poses, phonics letters, astrology/kundali symbols, NCLEX/medical imagery, or unrelated app UI unless that category is the target product or the task explicitly asks for it.
     - For every product-bearing reference element, the image prompt must include the replacement from `product_element_map`. Do not rely on generic instructions like "adapt to our product."
...
4. Generate variations.
   ...
   - Generate and upload in strict sequence. Create variation 1 first and make it the closest reference-faithful output. Only after variation 1 passes QA, create variation 2, then variation 3, etc. Filenames and ClickUp attachments must use the ClickUp task name plus a plain 1-based variation number: `<Task Name>-1`, `<Task Name>-2`, `<Task Name>-3`. For example, task `14021` must produce `14021-1.png`, `14021-2.png`, and `14021-3.png`. `producer_runs.outputs[].variation_id` and `generation_name` should use the same extensionless names (`14021-1`, `14021-2`, `14021-3`). Never reorder outputs by upload time, attachment order, visual preference, or newest-first UI order.
...
5. Quality gate before upload.
   ...
   - Reject and regenerate once if any of these fail:
     ...
     - output changes only the text/product name but leaves source-product visual artifacts, props, icons, category labels, demonstrations, packshots, app screens, or activity names that do not belong to the canonical product
...
6. Upload and update systems.
   ...
   - Use final-only filenames shaped like `<Task Name>-<variation_number>.<ext>`, where `<Task Name>` is the ClickUp task name and `<variation_number>` is `1`, `2`, `3`, etc. Example: task `14021` outputs `14021-1.png`, `14021-2.png`, `14021-3.png`. Do not use `RUN` ids, `V01` labels, or `_final` suffixes in uploaded filenames.
