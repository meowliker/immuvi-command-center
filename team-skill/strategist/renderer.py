...
def render_markdown(memory_json: dict) -> str:
    ...
    # Use whichever agent CLI is installed (claude OR codex). Mac Mini has
    # only Codex; laptop has both. The worker's build_agent_cmd handles the
    ...
    if r.returncode != 0:
        ...
        raise RenderError(f"{agent} exit {r.returncode}: {tail}")
