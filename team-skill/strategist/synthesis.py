...
def synthesise_task(bundle: dict) -> dict:
    """Run a skill-agent CLI once on a task bundle. Return the parsed JSON brief.

    Uses whichever agent CLI is installed (claude OR codex) via the worker's
    build_agent_cmd dispatcher — so the strategist works on Mac Mini (codex-
    only) as well as the laptop (both).
    """
    prompt = _build_prompt(bundle)
    try:
        from tools.classify_worker import build_agent_cmd
        cmd = build_agent_cmd(prompt, prefer="claude")
    except Exception:
        # Fallback for direct invocation outside the worker (tests, manual)
        cmd = ["claude", "-p", prompt, "--permission-mode", "bypassPermissions"]
    agent = cmd[0] if cmd[0] == "claude" else "codex"
    try:
        r = subprocess.run(cmd, capture_output=True, text=True,
                           timeout=CLAUDE_TIMEOUT_SECONDS)
    except subprocess.TimeoutExpired:
        raise SynthesisError(
            f"{agent} timed out after {CLAUDE_TIMEOUT_SECONDS}s")
    if r.returncode != 0:
        tail = (r.stderr or r.stdout or "")[-500:]
        raise SynthesisError(f"{agent} exit {r.returncode}: {tail}")
