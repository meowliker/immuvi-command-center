    ...
    def _execute_producer_run(self, prod_run: dict) -> None:
        ...
        try:
            ...
            _prompt = (
                ...
                "7. Build reference_anatomy, reference_layout_spec, and text_scale_map before generation. Include subject_type, subject_presence, setting, composition, overlay_system, copy_density, product_presence, canvas dimensions/aspect ratio, visual section count/order, each section bounding box as x/y/w/h percentages, photo/banner/lower-third/headline heights, margins/padding, each text block bounding box as percentages, line count, casing, alignment, weight/style, approximate font-size-to-canvas-height ratio, and font-size-to-section-height ratio. If exact pixels are unavailable, estimate visually and label as estimated. Treat text size and overlay proportions as hard constraints.\n"
                ...
                "11. Preserve reference anatomy, not just theme. Reference anatomy wins over offer/CTA prominence when they conflict. Preserve measured text scale, line count, section heights, margins/padding, and overlay bounding boxes; do not enlarge compact text into poster headlines or change text size because the offer feels important. If the reference has a human subject, include a comparable visible human subject in similar scale/role/pose region; do not replace them with a backpack, chair, desk, hallway, worksheet, empty classroom, playground, or product shot. For BREAKING NEWS references, use only three sections: photo + thin red/blue BREAKING NEWS band + one compact white headline strip. No green/red CTA bar, product mockup, oversized poster headline, large white copy block, icon/product row, or separate FREE TODAY button. If the offer cannot fit inside the same 1-2 line white headline strip, omit the offer.\n"
                ...
                "13. Quality gate each image before upload — persona match, readable typography, reference-anatomy match, measured layout match, text-scale match, mechanic match, brand fit, not generic. Reject if reference_layout_spec or text_scale_map is missing/uncertain, if section order or bounding boxes materially drift from the measured spec, if any major text block is materially larger/smaller than reference, grows more than about 20-25% taller than the mapped text box, changes line count/alignment/casing, or uses visibly different padding/margins. For BREAKING NEWS references, reject any image with no comparable visible human subject when the reference has one, a separate CTA/footer/button, a bottom poster block, oversized white headline/poster text, or more than three visual sections instead of photo + news band + one compact white headline strip.\n"
                ...
                "inspiration_dimensions, aspect_ratio, reference_anatomy, reference_layout_spec, text_scale_map, prompt, image_model, quality_gate. "
